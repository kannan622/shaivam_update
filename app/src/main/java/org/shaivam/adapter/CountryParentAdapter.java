...
public class CountryParentAdapter extends RecyclerView.Adapter<CountryParentAdapter.ViewHolder> {
    private List<Thirumurai> mDataset;
    private Context context;
    private LayoutInflater inflater;
    private static CountryChildAdapter adapter;
    ...
    public CountryParentAdapter(Context context, List<Thirumurai> thirumuraiDetails) {
    ...
    public void onBindViewHolder(final ViewHolder holder, final int position) {
        ...
        if (ds != null) {
            adapter = new CountryChildAdapter(context, thirumuraiDetails);
                    ...
                    if (currentPosition == holder.getPosition()) {
                        ...
                        try {
                            ...
                            List<Thirumurai_songs> thirumurai_songs = MyApplication.repo.repoThirumurai_songs
                                    .getCountryByThirumurai_Id(ds.getId());

                            for (int j = 0; j < thirumurai_songs.size(); j++) {
                                if (thirumurai_songs.get(j) != null &&
                                        !thirumurai_songs.get(j).getCountry().equalsIgnoreCase(""))
