...
public class TamilSoftKeyboard extends InputMethodService
    ...
    public View onCreateInputView() {
        mInputView = (KeyboardView) getLayoutInflater().inflate(
                R.layout.input, null);
//        mInputView.setBackgroundResource(R.drawable.background_home);
...
//                }
            ...
            default: {
                switch (TamilKeyboard.CURRENT_LAYOUT) {
                    case TamilKeyboard.KEYBOARD_TAMIL99:
                        if (mAlt && TamilKeyboardConstants.T99_ALT_MAP.containsKey(keyCode)) {
                            handleCharacter(TamilKeyboardConstants.T99_ALT_MAP.get(keyCode), null);
                            mAlt = event.isAltPressed();
                            mShift = event.isShiftPressed();
                            return true;
                        }
                        if (mShift && TamilKeyboardConstants.T99_SHIFT_MAP.containsKey(keyCode)) {
                            onKey(TamilKeyboardConstants.T99_SHIFT_MAP.get(keyCode), null);
                            mAlt = event.isAltPressed();
                            mShift = event.isShiftPressed();
                            return true;
                        }
                        if (TamilKeyboardConstants.T99_CODE_MAP.containsKey(keyCode)) {
                            onKey(TamilKeyboardConstants.T99_CODE_MAP.get(keyCode), null);
                            mAlt = event.isAltPressed();
                            mShift = event.isShiftPressed();
                            return true;
                        }
                    ...
                    case TamilKeyboard.KEYBOARD_PHONETIC:
                    case TamilKeyboard.KEYBOARD_PHONETIC_ENGLISH:
                        if (TamilKeyboardConstants.PHONETIC_KEY_CODE_MAP.containsKey(keyCode)) {
                            onKey(TamilKeyboardConstants.PHONETIC_KEY_CODE_MAP.get(keyCode), null);
                            mAlt = event.isAltPressed();
                            mShift = event.isShiftPressed();
                            return true;
                        }
            ...
            try {
                ...
                if (isInputViewShown()) {
                    if (mInputView.isShifted()) {
                        ...
                        if (TamilKeyboardConstants.PHONETIC_SHIFTED_KEYS.containsKey(primaryCode)) {
                            primaryCode = TamilKeyboardConstants.PHONETIC_SHIFTED_KEYS
                                    .get(primaryCode);
                        }
                ...
                if (isShifted || mShift) {
                    if (TamilKeyboardConstants.PHONETIC_SHIFTED_KEYS.containsKey(primaryCode)) {
                        primaryCode = TamilKeyboardConstants.PHONETIC_SHIFTED_KEYS.get(primaryCode);
                    }
                ...
                } else if (TamilKeyboardConstants.KURIL_EXECPT_AGARAM.contains(primaryCode)) {
                    if (mPrevChar == 3021) {
                        handleBackspace();
                        primaryCode = TamilKeyboardConstants.KURIL_SYMBOLS_MAP
                                .get(primaryCode);
                    } else if (TamilKeyboardConstants.KURIL_SYMBOLS.contains(mPrevChar)
                            && mPrevChar == TamilKeyboardConstants.KURIL_SYMBOLS_MAP
                            .get(primaryCode)) {
                        handleBackspace();
                        primaryCode = TamilKeyboardConstants.NEDIL_SYMBOLS_MAP
                                .get(primaryCode);
                        ...
                        handleBackspace();
                        primaryCode = TamilKeyboardConstants.UYIR_NEDIL_MAP.get(primaryCode);
                    }
                    getCurrentInputConnection().commitText(
                            String.valueOf((char) primaryCode), 1);
                    mText.append(String.valueOf((char) primaryCode));
